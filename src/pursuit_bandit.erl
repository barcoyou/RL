...
-module(pursuit_bandit).
-author('barcojie@gmail.com').

%%% -------------------------------------------------------------------------
%%% This is an agent using Pursuit method to solve bandit  
%%% problem which is introduced in chapter 2.9 
...
%%        for the action-probability update
...
model(State, {Action, Reward}) ->
	{Qt, Act_Times, Action_Prob} = update(State, Action, Reward),
...
update(State, Action, Reward) ->
	update(State#state.qt, State#state.action_times, State#state.action_prob, Action, Reward, State#state.param, [], [], 1).

update([], [], Prob, _Action, _Reward, Beta, Qt, At, _N) ->
	Qqt = lists:reverse(Qt), 
	Aat = lists:reverse(At),
	Max_Index = string:str(Qqt, [lists:max(Qqt)]),
	{Qqt, Aat, prob_update(Prob, Beta, Max_Index)};
update([QtH|QtT],[ATH|ATT],Prob, Action, Reward, Beta, Qt, At, N) ->
	if
		N == Action ->
			update(QtT, ATT, Prob, Action, Reward, Beta, [QtH+(Reward-QtH)/(ATH+1)|Qt],[ATH+1|At], N+1);%average sampling method
		true ->
			update(QtT, ATT, Prob, Action, Reward, Beta, [QtH|Qt], [ATH|At], N+1)
	end.
...
prob_update(Prob, Beta, Index) ->
	prob_update(Prob, Beta, Index, [], 1).
prob_update([], _Beta, _Index, Pt, _N) ->
	lists:reverse(Pt);
prob_update([ProbH|ProbT], Beta, Index, Pt, N) ->
	if
		N==Index ->
			prob_update(ProbT, Beta, Index, [ProbH+Beta*(1-ProbH)|Pt], N+1);
		true ->
			prob_update(ProbT, Beta ,Index, [ProbH+Beta*(0-ProbH)|Pt], N+1)
	end.
